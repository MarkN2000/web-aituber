...
ユーザー辞書の試聴では、入力中のカタカナを`POST /audio_query`へ渡し、生成された全アクセント句のモーラを連結した読みが入力と一致する場合に限り、モーラを1単語のアクセント句へまとめる。休止や疑問表現を含む音声クエリは対象外とする。辞書のアクセント位置0は平板を表すため末尾モーラの位置へ変換し、`POST /mora_pitch`で指定位置に対応する音高を再計算してから`POST /synthesis`へ渡す。その他のAudioQuery項目はエンジンの応答をそのまま保持し、ユーザー辞書への追加・更新は行わない。
