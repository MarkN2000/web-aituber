...
ユーザー辞書の試聴では、入力中のカタカナを`POST /audio_query`へ渡し、単一のアクセント句と入力した読みが一致する場合に限り、`accent`だけを入力値に対応する位置へ置き換えて`POST /synthesis`へ渡す。辞書のアクセント位置0は平板を表すため、AudioQueryでは末尾モーラの位置へ変換する。その他のAudioQuery項目はエンジンの応答をそのまま保持し、ユーザー辞書への追加・更新は行わない。
