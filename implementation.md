...
メイン画面URLに値を問わず`debug`クエリパラメータが存在する場合だけ、デバッグ機能をまとめて有効にする。右手配下の食事アンカーへ座標軸と設定サイズの枠を追加し、画面右上のオーバーレイへ実際に選択された身体モーションのファイル名と待機・感情の種別、VRMへ要求した基本表情を表示する。モーションと要求表情の状態が変わった時だけDOMを更新し、モデル側の表情対応有無、口パクの口形、瞬きは表示対象にしない。設定変更は管理APIで設定ファイルを再読み込みした後、デバッグ表示中のメイン画面をブラウザで再読み込みして反映する。
