...
サーバーPCとスマートフォンを同じWi-Fiへ接続し、`ipconfig`でサーバーPCのIPv4アドレスを確認します。IPv4アドレスが`192.168.1.20`の場合、スマートフォンから次のURLを開きます。

- メイン画面: `http://192.168.1.20:3000/`
- 簡易入力画面: `http://192.168.1.20:3000/input`

サーバーは`0.0.0.0:3000`で待ち受けます。Windowsファイアウォールでは、プライベートネットワークのTCP 3000番への受信だけを許可してください。`0.0.0.0`は待受設定用のため、ブラウザのURLには使用しません。
