...
設定ファイルの候補は`idle_speech.entries`へ`[{"emotion":"neutral","text":"ひと休み……"},{"emotion":"happy","text":"何かお話ししよう！"}]`のように保存します。以前の単独の`idle_speech.emotion`・`idle_speech.text`を設定済みの場合は、その組を`entries`配列に入れてください。旧形式の自動変換は行いません。
