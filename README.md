...
右上の「モーションテスト」を開くと、感情と候補（ランダムまたは個別ファイル）を選んで再生できます。同じ候補も先頭から再生し直せます。終了時または「待機に戻す」で通常表情・待機モーションへ戻ります。折りたたんでも再生と選択は維持します。

モデル読み込み中、準備中、投稿処理中はテストできません。テスト中に投稿処理が始まると自動で中断します。未登録・読み込み失敗の感情は再生できません。この操作でLLM・TTSは呼び出さず、設定や会話履歴も変更しません。
