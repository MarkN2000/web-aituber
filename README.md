...
空の`url`が入力欄です。JSONにはコメントを書けないため、URLが空の間は未設定として扱います。通常質問は利用できますが、食事投稿は「食事モーションが設定されていません」で受け付けません。発話開始時刻が未設定の古い設定は、起動時に消去開始時刻を引き継いで補完します。設定済みの値は上書きしません。編集するのは使用中の`config.json`であり、`config.example.json`だけを編集しても反映されません。

`url`は食べるモーションのファイルです。`consume_at_ms`は再生開始から食べ物の画像を消し始めるまでの時間です。`speech_start_ms`は感想を喋り始めるまでの時間で、消去とは独立して設定できます。どちらも0以上の整数ミリ秒です。画像は400ミリ秒かけて縮小して消します。`duration_ms`は食事演出全体の時間です。発話中もモーションは元の速度で1回再生し、演出が終わると待機へ戻ります。感想が短くてもモーションは演出終了まで続きます。消去が完了するよう、`duration_ms`は`consume_at_ms + 400`以上にしてください。
...
音声設定の変更、このアプリからの辞書更新、新バージョンでの起動時にはキャッシュを全削除します。外部でモデルや辞書を更新した場合は、管理画面の「音声設定」→「音声キャッシュを削除」を押してください。再生中の音声には影響しません。
...
右上の「モーションテスト」を開くと、種類（待機・各感情）と候補（ランダムまたは個別ファイル）を選んで再生できます。「待機」は待機モーション、「通常」はneutralの感情モーションです。同じ候補も先頭から再生し直せます。終了時または「待機に戻す」で通常表情・待機モーションへ戻ります。折りたたんでも再生と選択は維持します。

モデル読み込み中、準備中、投稿処理中はテストできません。テスト中に投稿処理が始まると自動で中断します。未登録・読み込み失敗の種類は再生できません。この操作でLLM・TTSは呼び出さず、設定や会話履歴も変更しません。
...
管理画面の「モーション」タブにある「感想を喋り始める時刻」で、食事開始から発話までの秒数を指定できます。モーション終了後に発話を始める設定も可能です。

「モーションファイルの削除」では不要なVRMAを選択して削除できます。割り当て中のファイルは、候補から外して「モーション設定を保存」した後に削除してください。確認後の削除は取り消せません。
...
管理画面の「AI設定」→「待機中のセリフ」で、共通の最短・最長待機秒数を設定し、「セリフを追加」で感情とセリフの組を増やしてください。「AI設定を保存」で確定します。発話するたびに候補からランダムに1組選ぶため、同じ候補が続くこともあります。改行は同じ候補内の文章です。初期状態は無効です。30〜90秒なら毎回その範囲から待ち時間を選び、同じ秒数なら固定間隔になります。

設定ファイルの候補は`idle_speech.entries`へ`[{"emotion":"neutral","text":"ひと休み……"},{"emotion":"happy","text":"何かお話ししよう！"}]`のように保存します。以前の単独の`idle_speech.emotion`・`idle_speech.text`を設定済みの場合は、更新後の初回起動時にその組を`entries`配列の1件へ自動変換・保存します。有効・無効、待機時間、その他の設定値は保持します。`APP_CONFIG_FILE`で指定したファイルも対象です。設定の検証や保存に失敗した場合は元ファイルを保持して起動エラーとします。

待機発話を終えたらカウントし直します。投稿が来た場合は待機発話を中断して投稿を優先します。準備中モードでは発話せず、会話履歴にも追加しません。終了判定には通常の回答と同じくサーバー側の音声時間を使い、通常回答の再生が残っている端末では待機発話を見送ります。
